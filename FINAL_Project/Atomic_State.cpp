	...
	if (m_modelState == STATE::INIT) {
		...
		if (m_type != 1) {
			...
			if (m_type == 0) {
				...
				if (GLOBAL_VAR->mBufferSize(0, m_pk, &GLOBAL_VAR->p_buffer) >= GLOBAL_VAR->m_maxbuffer[m_type]) {
					m_modelState = STATE::WAIT;
...
void Atomic_State::m_dataUpdate() {
	...
	if (GLOBAL_VAR->SQLConnect == true) {
		GLOBAL_VAR->pgconn->SendQuery("INSERT INTO \"object_state_list" + std::to_string(GLOBAL_VAR->scenario_num) + "\" (project_id, object_id, object_state, state_start_time, state_end_time) VALUES("+std::to_string(GLOBAL_VAR->scenario_num)+", "+ std::to_string(m_pk) + ", '" + getState2Str(m_modelState) + "', " + std::to_string(m_current_time) + ", " + std::to_string(m_endTime) + ")");
		GLOBAL_VAR->pgconn->SendQuery("INSERT INTO \"state_time_list" + std::to_string(GLOBAL_VAR->scenario_num) + "\" (project_id, object_id, \"current_time\", init_time, active_time, error_time, wait_time) VALUES(" + std::to_string(GLOBAL_VAR->scenario_num) + ", " + std::to_string(m_pk) + ", " + std::to_string(m_endTime) + ", " + std::to_string(timeStore[0]) + ", " + std::to_string(timeStore[1]) + ", " + std::to_string(timeStore[2]) + ", " + std::to_string(timeStore[3]) + ")");
		GLOBAL_VAR->pgconn->SendQuery("INSERT INTO \"state_rate_list" + std::to_string(GLOBAL_VAR->scenario_num) + "\" (project_id, object_id, \"current_time\", init_rate, active_rate, error_rate, wait_rate) VALUES(" + std::to_string(GLOBAL_VAR->scenario_num) + ", " + std::to_string(m_pk) + ", " + std::to_string(m_endTime) + ", " + std::to_string(100 * timeStore[0] / m_endTime) + ", " + std::to_string(100 * timeStore[1] / m_endTime) + ", " + std::to_string(100 * timeStore[2] / m_endTime) + ", " + std::to_string(100 * timeStore[3] / m_endTime) + ")");
		GLOBAL_VAR->pgconn->SendQuery("INSERT INTO \"buf_count_list" + std::to_string(GLOBAL_VAR->scenario_num) + "\" (project_id, object_id, object_type, \"current_time\", buffer_count, stock_count) VALUES(" + std::to_string(GLOBAL_VAR->scenario_num) + ", " + std::to_string(m_pk) + ", '" + getModel2Str(m_type) + "', " + std::to_string(m_endTime) + ", " + std::to_string(GLOBAL_VAR->mBufferSize(0, m_pk, &GLOBAL_VAR->p_buffer)) + ", " + std::to_string(GLOBAL_VAR->BufferSize(m_pk, &GLOBAL_VAR->stock)) + ")");
	}
